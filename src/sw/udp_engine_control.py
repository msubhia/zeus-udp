...
class connection_manager_sw:
    ...
    def _hash_fun_ip_port(ip, port):
        """
        Compute a 16-bit hash from IP address and UDP port.
        
        This is a bit-accurate Python implementation of the SystemVerilog hash function
        used in the hardware. Uses multiplicative hashing with Knuth's constant.
    ...
    def write(self, ipAddr, udpPort, bind):
        ...
        if bind:
            ...
            for w in range(self.WAYS):
                if (self.my_hash_table_vlds[w][hash_key] == 1) and (self.my_hash_table_ipAddr[w][hash_key] == ipAddr) and (self.my_hash_table_udpPort[w][hash_key] == udpPort):
            ...
            for w in range(self.WAYS):
                if self.my_hash_table_vlds[w][hash_key] and (self.my_hash_table_ipAddr[w][hash_key] == ipAddr) and (self.my_hash_table_udpPort[w][hash_key] == udpPort):
...
class udp_engine_controller:
    ...
    def configure(self, src_macAddr, src_ipAddr, src_udpPort, dst_macAddr):
        ...
        """
        self.tx_disable()
        self._write_confirmed(self.addr_csr_udp_engine_100g__myConfig_macAddr_upper, (src_macAddr >> 32) & 0xFFFFFFFF)
        self._write_confirmed(self.addr_csr_udp_engine_100g__myConfig_macAddr_lower, src_macAddr & 0xFFFFFFFF)
        self._write_confirmed(self.addr_csr_udp_engine_100g__myConfig_ipAddr, src_ipAddr)
        self._write_confirmed(self.addr_csr_udp_engine_100g__myConfig_udpPort, src_udpPort)
        self._write_confirmed(self.addr_csr_udp_engine_100g__myConfig_macAddr_upper_dst, (dst_macAddr >> 32) & 0xFFFFFFFF)
        self._write_confirmed(self.addr_csr_udp_engine_100g__myConfig_macAddr_lower_dst, dst_macAddr & 0xFFFFFFFF)
        self.tx_enable()
    ...
    def bind_connection(self, dst_ipAddr, dst_udpPort):
        ...
        """
        self._write_confirmed(self.addr_csr_udp_engine_100g__connManager_wr_ip_addr, dst_ipAddr)
        self._write_confirmed(self.addr_csr_udp_engine_100g__connManager_wr_port, dst_udpPort)
...
class udp_model:
    ...
    def __init__(self, connection_manager, MY_CONFIG_DST_MAC, MY_CONFIG_SRC_MAC, MY_CONFIG_SRC_IP, MY_CONFIG_SRC_PORT):
        self.connection_manager = connection_manager
    ...
    def _generate_random_packet(self):
        ...
        if self.connection_manager.existing_connection_ids and random.random() < 0.9:
            connection_id = random.choice(self.connection_manager.existing_connection_ids)
        else:
    ...
    @staticmethod
    def _build_ipv4_header(payload_length_bytes, 
                        src_ip, dst_ip,
                        dscp, ecn,
                        identification,
                        flags, frag_offset,
    ...
    def _model_udp(self, connection_id, payload_length_bytes, payload_bytes):
        ...
        udp_header = (
            ...
            (udp_model._swap_bytes(payload_length_bytes + udp_model.UDP_HEADER_BYTES, 2)  << 32)   |
    ...
    def generate_test(self):
        ...
        """
        connection_id, payload_length_bytes, payload_bytes = self._generate_random_packet()
        expected_output = self._model_udp(connection_id, payload_length_bytes, payload_bytes)
