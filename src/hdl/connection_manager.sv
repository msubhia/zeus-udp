...
module connection_manager #(
  ...
  logic [CONN_ID_WIDTH-1:0] m00_axis_fw_lookup_connectionId_pipe;
  logic m00_axis_fw_lookup_hit_pipe;
  logic m00_axis_fw_lookup_valid_pipe;

  // output logic
  always_comb begin
    s00_axis_fw_lookup_ready             = 1'b1;
    m00_axis_fw_lookup_valid_pipe        = fw_valid_pipe[BRAM_LATENCY+1];

    m00_axis_fw_lookup_hit_pipe          = 1'b0;
    m00_axis_fw_lookup_connectionId_pipe = '0;

    for (int w = 0; w < WAYS; w++) begin
      if (fw_valid[w] && (fw_tag[w] == {fw_ipAddr_pipe[BRAM_LATENCY+1], fw_udpPort_pipe[BRAM_LATENCY+1]})) begin
        m00_axis_fw_lookup_hit_pipe |= 1'b1;
        m00_axis_fw_lookup_connectionId_pipe |= {w[WAYS_LOG-1:0], fw_hash_idx_pipe[BRAM_LATENCY+1]};
      end
    end
  end

  always_ff @(posedge s00_axis_fw_lookup_aclk) begin
    m00_axis_fw_lookup_connectionId <= m00_axis_fw_lookup_connectionId_pipe;
    m00_axis_fw_lookup_hit          <= m00_axis_fw_lookup_hit_pipe;
    m00_axis_fw_lookup_valid        <= m00_axis_fw_lookup_valid_pipe;
  end
